...
class Prepare:
    ...
    def exportToJSON(self):
        export_to_JSON.export_to_JSON(
            self.cytoscape_node_data, self.cytoscape_edge_data)
    ...
    def prepare_rg_node(self):
        resource_group_prepare_node.resource_group_prepare_node(self.region_name,
                                                                self.account_id, self.cytoscape_node_data)

    def find_sfn_connection(self):
        sfn_find_connection.sfn_find_connection(
            self.cytoscape_node_data, self.cytoscape_edge_data)

    def find_edge_lambda_to_sns(self):
        edge_lambda_sns_find.edge_lambda_sns_find(
            self.cytoscape_edge_data, self.region_name)

    def find_edge_lambda_to_ddb(self):
        edge_lambda_ddb_find.edge_lambda_ddb_find(
            self.cytoscape_edge_data, self.region_name)
    ...
    def find_edge_sns_to_lambda(self):
        edge_sns_to_lambda_find.edge_sns_to_lambda_find(
            self.cytoscape_edge_data)
    ...
    def write_to_dynamoDB(self, requestID, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY):
        export_to_s3.write_to_dynamo_db(
            requestID, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY)
